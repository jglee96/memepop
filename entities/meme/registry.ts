...
const memeRegistry: ReadonlyArray<Meme> = [
  {
    ...
    title: "아빠도 이제 한계다",
    description:
      "‘아빠도 이제 한계다’ 밈은 오랫동안 참고 버텨온 사람이 한 번에 감정을 폭발시키는 장문 메시지 형식을 패러디하는 한국어 밈입니다. 핵심은 단순한 화풀이가 아니라, ‘충분히 기다려줬다’는 누적된 서사를 장황하게 쌓은 뒤 마지막에 단호한 통보로 마무리하는 리듬입니다. 그래서 읽는 사람은 진지한 호소처럼 시작된 문장이 점점 과장된 드라마 톤으로 커지는 데서 웃음을 느끼게 됩니다.\n\n실제 사용에서는 가족 관계에만 제한되지 않고 회사, 팀플, 게임, 커뮤니티 운영처럼 반복된 실수나 책임 회피가 누적된 상황을 표현할 때 자주 쓰입니다. 특히 ‘OO 탓 XX 탓 하지 마라’ 같은 문장 구조를 현재 맥락에 맞게 바꾸면 밈의 인지 포인트가 빠르게 살아납니다. 즉, 개인 공격보다도 ‘참을 만큼 참았다’는 과장된 연출을 통해 상황을 풍자하는 장르에 가깝습니다.\n\nMemePop에서는 사용자가 짧게 상황만 입력하면, 원본 밈의 서사 구조를 유지한 장문 한 메시지 형태로 자동 변형합니다. 결과는 서비스에 저장되지 않고 즉시 반환되며, 사용자는 복사해서 외부 플랫폼에 바로 붙여 넣어 사용할 수 있습니다.",
    useCases: [
