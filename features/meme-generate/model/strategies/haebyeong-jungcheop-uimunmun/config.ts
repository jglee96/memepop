export type LayerPattern = (inner: string) => string;

export const NESTING_UNIT_PATTERN =
  /(행동인지|것인지|무엇인지|공정한지|않는지|않은지|될지|되는지|있는지|맞는지|아닌지|가능한지|옳은지|적절한지|괜찮은지|되겠습니까)/g;
export const INVALID_OUTPUT_PATTERN = /(https?:\/\/|www\.|```|[#*_`[\]{}<>])/i;
export const LOW_VARIETY_PATTERN =
  /(보고드려도 되는지|문의드려도 되는지|검증받아도 되는지|확인받아도 되는지)/g;
export const PREMATURE_FINAL_ENDING_PATTERN = /되겠습니까/g;

export const CORE_PATTERNS: ReadonlyArray<(count: number) => string> = [
...
export const NON_FINAL_WRAPPERS: ReadonlyArray<LayerPattern> = [
...
export const FINAL_WRAPPERS: ReadonlyArray<LayerPattern> = [
